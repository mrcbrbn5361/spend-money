...
import { setItems } from './redux/slices/MainSlice';
...
function App() {
  ...
  let totalSpentMoney = Object.values(info).reduce((res, cur) => res + cur.count * cur.price, 0);
  let restMoney = money - totalSpentMoney;
  ...
  return (
    <div className="App">
      ...
      <div className="container p-0">
        ...
        <div className="countzz">${restMoney.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</div>
        <div className="row mt-2 px-2">
          {items.map((item) => (
            <Item key={item.id} bank={restMoney} base={item} />
          ))}
        </div>
